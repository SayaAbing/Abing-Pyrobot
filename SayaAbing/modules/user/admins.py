...
from .help import *
from SayaAbing.utils.misc import extract_user, extract_user_and_reason, list_admins
...
async def pin_message(client: Client, message):
    ...
    if not bot.can_pin_messages:
        return await SayaAbing.edit("I don't have enough permissions")
...
async def kick_user(client: Client, message: Message):
    ...
    if not bot.can_restrict_members:
        return await SayaAbing.edit("I don't have enough permissions")
    if not user_id:
        return await SayaAbing.edit("I can't find that user.")
    if user_id == client.me.id:
        return await SayaAbing.edit("I can't kick myself.")
    if user_id == DEVS:
        return await SayaAbing.edit("I can't kick my developer")
    if user_id in (await list_admins(client, message.chat.id)):
        return await SayaAbing.edit("I can't kick an admin, You know the rules, so do i.")
    mention = (await client.get_users(user_id)).mention
    msg = f"""
**Kicked User:** {mention}
**Kicked By:** {message.from_user.mention if message.from_user else 'Anon'}"""
    ...
    try:
        ...
        return await SayaAbing.edit("**Maaf Anda Bukan admin**")
...
async def promotte(client: Client, message: Message):
    ...
    if message.command[0][0] == "f":
        ...
        return await SayaAbing.edit(f"Fully Promoted! {umention}")
    ...
    await SayaAbing.edit(f"Promoted! {umention}")
...
async def demote(client: Client, message: Message):
    ...
    if not user_id:
        return await SayaAbing.edit("I can't find that user.")
    if user_id == client.me.id:
        return await SayaAbing.edit("I can't demote myself.")
    await message.chat.promote_member(
        user_id,
        privileges=ChatPrivileges(
    ...
    umention = (await client.get_users(user_id)).mention
    await SayaAbing.edit(f"Demoted! {umention}")
