...
from SayaAbing import BOTLOG_CHATID, LOGGER
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Abing-Pyrobot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**SayaAbing-Pyrobot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="SayaAbing/resources/logo.jpg",
