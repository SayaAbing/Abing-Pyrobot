...
from SayaAbing import TEMP_SETTINGS
from SayaAbing.helpers.adminHelpers import DEVS
from SayaAbing.helpers.basic import edit_or_reply
from SayaAbing.helpers.SQL.globals import addgvar, gvarstatus
from SayaAbing.helpers.tools import get_arg

from .help import *

DEF_UNAPPROVED_MSG = (
    "PMSecurity of ㅤ˹ ΛBIПG-PYЯӨBӨƬ ˼ !\n"
...
async def incomingpm(client: Client, message: Message):
    try:
        from SayaAbing.helpers.SQL.globals import gvarstatus
        from SayaAbing.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from SayaAbing.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
    ...
    if message.chat.id in DEVS:
        try:
            ...
            await client.send_message(
                message.chat.id,
                f"<b>Menerima Pesan!!!</b>\n{message.from_user.mention} <b>Terdeteksi Developer Abing-Pyrobot🥰</b>",
...
async def approvepm(client: Client, message: Message):
    try:
        from SayaAbing.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from SayaAbing.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from SayaAbing.helpers.SQL.globals import addgvar
    ...
    SayaAbing = await cust_msg.edit("`Processing...`")
    if input_str and not input_str.isnumeric():
        return await SayaAbing.edit("**Harap masukan angka untuk PM_LIMIT.**")
    addgvar("PM_LIMIT", input_str)
    await SayaAbing.edit(f"**Set PM limit to** `{input_str}`")
...
async def stpmpt(client: Client, cust_msg: Message):
    ...
    try:
        import SayaAbing.helpers.SQL.globals as sql
    except AttributeError:
        await cust_msg.edit("**Running on Non-SQL mode!**")
        return
    SayaAbing = await cust_msg.edit("`Processing...`")
    ...
    if not message:
        return await SayaAbing.edit("**Mohon Reply Ke Pesan**")
    msg = message.text
    sql.addgvar("unapproved_msg", msg)
    await SayaAbing.edit("**Pesan Berhasil Disimpan Ke Room Chat**")
...
async def gtpmprmt(client: Client, cust_msg: Message):
    ...
    custom_message = sql.gvarstatus("unapproved_msg")
    if custom_message is not None:
        await SayaAbing.edit("**Pesan PMPERMIT Yang Sekarang:**" f"\n\n{custom_message}")
    else:
...
async def gtpmprmt(client: Client, cust_msg: Message):
    ...
    if custom_message is None:
        await SayaAbing.edit("**Pesan PMPERMIT Anda Sudah Default**")
    else:
        sql.delgvar("unapproved_msg")
        await SayaAbing.edit("**Berhasil Mengubah Pesan Custom PMPERMIT menjadi Default**")
