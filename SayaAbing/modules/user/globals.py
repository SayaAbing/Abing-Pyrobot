...
from SayaAbing import *
from SayaAbing.helpers.adminHelpers import DEVS, WHITELIST
from SayaAbing.helpers.basic import edit_or_reply
from SayaAbing.helpers.PyroHelpers import get_ub_chats
from SayaAbing.utils import extract_user, extract_user_and_reason
...
def globals_init():
    try:
        ...
        sql = import_module("SayaAbing.helpers.SQL.gban_sql")
        sql2 = import_module("SayaAbing.helpers.SQL.gmute_sql")
...
async def gban_user(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        SayaAbing = await message.reply("`Gbanning...`")
    else:
        SayaAbing = await message.edit("`Gbanning....`")
    if not user_id:
        return await SayaAbing.edit("Mau Gban Siapa Hayoo!!!!.")
    if user_id == client.me.id:
        return await SayaAbing.edit("**Ngapain NgeGban Diri Sendiri Pepek**")
    if user_id in DEVS:
        return await SayaAbing.edit("**Gak Bisa Di Gban Kontol karena dia Yang Buat Aku 🗿**")
    if user_id in WHITELIST:
        return await SayaAbing.edit(
            "**Kau Gak Bisa Kontol Gban Dia Karena Dia Adalah admin @ab1ngsupport 😡**"
        )
    if user_id:
        try:
            user = await client.get_users(user_id)
        except Exception:
            return await SayaAbing.edit("`Harap tentukan pengguna yang valid!`")
    ...
    if not f_chats:
        return await SayaAbing.edit("**Anda tidak mempunyai GC yang anda admin 🥺**")
    er = 0
    done = 0
...
async def ungban_user(client: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message, sender_chat=True)
    if message.from_user.id != client.me.id:
        SayaAbing = await message.reply("`UnGbanning...`")
    else:
        SayaAbing = await message.edit("`UnGbanning....`")
    if not user_id:
        return await SayaAbing.edit("Saya tidak dapat menemukan pengguna itu.")
    if user_id:
        try:
            user = await client.get_users(user_id)
        except Exception:
    ...
    try:
        if user.id not in DEVS:
            await SayaAbing.edit("Memproses..")
            if not sql.is_gbanned(user.id):
                return await SayaAbing.edit("`User already ungban`")
        ...
        await SayaAbing.edit(f"**ERROR:** `{e}`")
        return
...
async def gbanlist(client: Client, message: Message):
    ...
    if not users:
        return await SayaAbing.edit("Belum Ada Jamet yang Di-Gban")
    gban_list = "**GBanned Users:**\n"
    count = 0
    for i in users:
        count += 1
        gban_list += f"**{count} -** `{i.sender}`\n"
    return await SayaAbing.edit(gban_list)
...
async def gmute_user(client: Client, message: Message):
    ...
    SayaAbing = await edit_or_reply(message, "`Processing...`")
    if args:
        try:
            user = await client.get_users(args)
        except Exception:
            await SayaAbing.edit(f"`Please specify a valid user!`")
            return
    elif reply:
    ...
    if user.id == client.me.id:
        return await SayaAbing.edit("**Ngapain NgeGmute Diri Sendiri Pepek**")
    if user.id in DEVS:
        return await SayaAbing.edit("**Gak Bisa Di Gmute Kontol karena dia Yang Buat Aku 🗿**")
    if user.id in WHITELIST:
        return await SayaAbing.edit(
            "**Mana Bisa Anjing Gmute Admin @ab1ngsupport 😡**"
        )
    try:
        replied_user = reply.from_user
        if replied_user.is_self:
            return await SayaAbing.edit("`Calm down anybob, you can't gmute yourself.`")
    except BaseException:
        pass

    try:
        if sql2.is_gmuted(user.id):
            return await SayaAbing.edit("`User already gmuted`")
        sql2.gmute(user.id)
        await SayaAbing.edit(f"[{user.first_name}](tg://user?id={user.id}) globally gmuted!")
        try:
...
async def ungmute_user(client: Client, message: Message):
    ...
    try:
        if not sql2.is_gmuted(user.id):
            return await SayaAbing.edit("`User already ungmuted`")
...
async def gmutelist(client: Client, message: Message):
    ...
    SayaAbing = await edit_or_reply(message, "`Processing...`")
    if not users:
        return await SayaAbing.edit("Belum Ada Jamet yang Di-Gmute")
    gmute_list = "**GMuted Users:**\n"
    count = 0
    for i in users:
        count += 1
        gmute_list += f"**{count} -** `{i.sender}`\n"
    return await SayaAbing.edit(gmute_list)
