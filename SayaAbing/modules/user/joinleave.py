...
async def join(client: Client, message: Message):
    SayaAbing = message.command[1] if len(message.command) > 1 else message.chat.id
    xxnx = await edit_or_reply(message, "`Processing...`")
    try:
        await xxnx.edit(f"**Berhasil Bergabung ke Chat ID** `{SayaAbing}`")
...
async def leave(client: Client, message: Message):
    ...
    try:
        ...
        await client.leave_chat(SayaAbing)
    except Exception as ex:
...
async def kickmeall(client: Client, message: Message):
    SayaAbing = await edit_or_reply(message, "`Global Leave from group chats...`")
    er = 0
    done = 0
