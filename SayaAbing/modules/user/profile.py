...
from SayaAbing.utils.misc import extract_user

from .help import *

flood = {}
profile_photo = "SayaAbing/modules/cache/pfp.jpg"
...
async def block_user_func(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await SayaAbing.edit("anda stress harap segera minum obat.")
...
async def setname(client: Client, message: Message):
    ...
    if len(message.command) == 1:
        ...
        try:
            ...
            await SayaAbing.edit(f"**Berhasil Mengubah Nama Telegram anda Menjadi** `{name}`")
...
async def set_bio(client: Client, message: Message):
    SayaAbing = await edit_or_reply(message, "`Processing . . .`")
    if len(message.command) == 1:
        return await SayaAbing.edit("Berikan teks untuk ditetapkan sebagai bio.")
    elif len(message.command) > 1:
        bio = message.text.split(None, 1)[1]
        try:
            await client.update_profile(bio=bio)
            await SayaAbing.edit(f"**Berhasil Mengubah BIO anda menjadi** `{bio}`")
        except Exception as e:
            await SayaAbing.edit(f"**ERROR:** `{e}`")
    else:
        return await SayaAbing.edit("Berikan teks untuk ditetapkan sebagai bio.")
